...
fn local_addr(listener: &TcpListener) -> Result<SocketAddr, String> {
    listener.local_addr().map_err(|e| format!("local_addr failed: {e}"))
}
...
fn establish_server(stream: TcpStream) -> Result<SecurePeerStream, String> {
    let mut stream = stream;
    authenticate_peer(&mut stream, TOKEN)
        .map_err(|e| format!("peer auth failed: {e}"))?;
...
fn transit_node_forwarder(source: SecurePeerStream, next_hop_pool: SharedPeerPool) -> Result<(), String> {
    ...
    loop {
        match source.read_secure_payload() {
            Ok(payload) => {
...
fn destination_deliver(mut peer: SecurePeerStream, target_addr: SocketAddr) -> Result<(), String> {
    let mut target = TcpStream::connect_timeout(&target_addr, Duration::from_millis(CONNECT_TIMEOUT_MS))
...
fn sealed_multi_hop_transit_reaches_destination_unchanged() -> Result<(), String> {
        ...
        let mut client = TcpStream::connect_timeout(&alice_local_addr, Duration::from_millis(CONNECT_TIMEOUT_MS))
