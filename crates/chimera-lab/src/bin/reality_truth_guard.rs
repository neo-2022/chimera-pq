...
fn main() {
    ...
    for (name, obj) in [
        ...
        {
            fail(&format!(
                "reality truth guard: GitHub SSH runtime slice mismatch in {name}"
            ));
        }
    }
...
fn validate_datapath_logic(ship: &serde_json::Map<String, Value>) -> Result<(), String> {
    ...
    let datapath_release_ok = ship
        ...
        .and_then(Value::as_bool)
        .ok_or_else(|| {
            "reality truth guard: invalid runtime_real_world_datapath_release_ok".to_string()
        })?;
    ...
    {
        return Err("reality truth guard: evidence kind value is invalid".to_string());
    }
    if ok_flag && !chimera_evidence {
        return Err("reality truth guard: datapath ok without CHIMERA evidence".to_string());
    }
    ...
    if external_ok + external_failed != external_total {
        return Err("reality truth guard: external reachability totals mismatch".to_string());
    }
    let probe_mode = ship
    ...
    if datapath_release_ok != expected_datapath_release_ok {
        return Err(
            "reality truth guard: datapath release flag does not match CHIMERA evidence"
                .to_string(),
        );
    }
    if ship.get("status").and_then(Value::as_str) == Some("ok") && !datapath_release_ok {
        return Err(
            "reality truth guard: status ok requires CHIMERA datapath release evidence".to_string(),
        );
    }
    if ci_snapshot {
        ...
        if evidence_kind != "ci_snapshot_contract" || chimera_evidence {
            return Err("reality truth guard: ci_snapshot evidence fields mismatch".to_string());
        }
        return Ok(());
    }
    ...
    if evidence_kind == "external_reachability_without_system_proxy" {
        if chimera_evidence || attempted || ok_flag {
            return Err(
                "reality truth guard: external reachability must not masquerade as datapath"
                    .to_string(),
            );
        }
        if !skipped_no_curl && !external_attempted {
            return Err(
                "reality truth guard: external reachability must be attempted when curl is available"
                    .to_string(),
            );
        }
        if external_attempted && external_total <= 0 {
            return Err(
                "reality truth guard: external reachability attempted with empty target totals"
                    .to_string(),
            );
        }
        if external_ok_flag && external_failed != 0 {
            return Err(
                "reality truth guard: external reachability ok with failed targets".to_string(),
            );
        }
        if !skipped_no_curl && error != "chimera_datapath_evidence_missing" {
            return Err(
                "reality truth guard: external reachability requires missing datapath evidence marker"
                    .to_string(),
            );
        }
        return Ok(());
    ...
    if !skipped_no_curl && !attempted {
        return Err(
            "reality truth guard: datapath must be attempted when CHIMERA evidence is available"
                .to_string(),
        );
    }
