...
fn run() -> Result<()> {
    ...
    match args.next().as_deref() {
        ...
        Some("parse-peer-metadata") => {
            let file = take_flag_value(&mut args, "--file")?;
            let metadata_url = take_flag_value(&mut args, "--metadata-url")?;
            peer_update::parse_peer_metadata_file(Path::new(&file), &metadata_url).map(|metadata| {
                println!("{}", metadata.version);
                println!("{}", metadata.archive);
                println!("{}", metadata.checksum);
                println!("{}", metadata.sha256);
            })
        }
        _ => {
            eprintln!(
                "usage: chimera-bootstrap <download|verify|extract|install|serve-release|parse-peer-metadata> --flags..."
            );
