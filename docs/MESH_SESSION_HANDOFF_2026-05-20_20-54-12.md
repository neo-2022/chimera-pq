...
## What Was Done (fact)
- Performed anti-monolith split for `mesh_cli` tests.
- Replaced single `tests.rs` with focused test modules:
  - `tests_json_contract.rs` for route-explain JSON contracts and failure-path JSON shape.
  - `tests_consistency_sources.rs` for consistency-source inference helpers.
- Wired new test modules via `mod.rs`; removed old `tests.rs`.

## Files Touched (fact)
- `crates/chimera-cli/src/mesh_cli/tests_json_contract.rs` (new)
- `crates/chimera-cli/src/mesh_cli/tests_consistency_sources.rs` (new)
- `crates/chimera-cli/src/mesh_cli/mod.rs`
- `crates/chimera-cli/src/mesh_cli/tests.rs` (deleted)
...
- `crates/chimera-cli/src/mesh_cli/tests_json_contract.rs`: `248` lines.
...
- `crates/chimera-cli/src/mesh_cli/route_explain_output.rs`: `74` lines.
...
## Next Step (planned)
- Continue MVP diagnostics by adding compact route-explain success/error snapshot stability checks while keeping each test module bounded.
