...
## Remote SIDE_A/Side B Peer Benchmark

Topology:

```text
SIDE_A app/probe -> SIDE_A peer-egress -> encrypted peer channel -> side_b peer -> side_b download-echo target
```
...
Remote SIDE_A retest after the in-place patch was initially blocked because SSH to `<stand-host-b-ip>:22` timed out before test startup. After SIDE_A reboot, the remote retest completed.
...
```text
event=transparent_flow_accepted destination=<stand-host-a-ip>:18603
event=transparent_direct_failed reason=direct connect failed: <stand-host-a-ip>:18603: connection timed out
event=transparent_route_selected route=gateway destination=<stand-host-a-ip>:18603
```
...
- Post in-place remote SIDE_A/side_b retest passed after SIDE_A reboot.
