...
run_control_case() {
  ...
  local tmp_dir bin_dir install_root cache_dir config_dir runtime_dir record output rc env_file
...
run_rejects_non_nft_override_case() {
  local tmp_dir bin_dir install_root cache_dir config_dir runtime_dir record output rc fake_nft
...
EOF
  ...
  if [[ -s "$record" ]]; then
    fail "non_nft_override: sudo should not be invoked"
  fi
