...
runtime_real_world_datapath_probe_attempted=false
runtime_real_world_datapath_probe_ok=false
runtime_real_world_datapath_probe_error="unknown"
...
runtime_probe_access_ssh_stand_required_for_live_probe=true
runtime_probe_access_ci_snapshot_targets_ok=false
...
if rg -q '"status":"ok"' docs/RUNTIME_DATAPATH_MULTIFLOW_SMOKE.json && rg -q '"network_state":"not_modified"' docs/RUNTIME_DATAPATH_MULTIFLOW_SMOKE.json && rg -q '"transit_ok":true' docs/RUNTIME_DATAPATH_MULTIFLOW_SMOKE.json && rg -q '"block_ok":true' docs/RUNTIME_DATAPATH_MULTIFLOW_SMOKE.json && rg -q '"direct_ok":true' docs/RUNTIME_DATAPATH_MULTIFLOW_SMOKE.json; then
...
if rg -q '"status":"ok"' docs/MESH_ROUTE_EXPLAIN.json && rg -q '"kind":"mesh_route_explain"' docs/MESH_ROUTE_EXPLAIN.json && rg -q '"initial_selected_peer":"peer#1"' docs/MESH_ROUTE_EXPLAIN.json && rg -q '"failover_selected_peer":"peer#1"' docs/MESH_ROUTE_EXPLAIN.json && rg -q '"cooldown_selected_peer":"peer#1"' docs/MESH_ROUTE_EXPLAIN.json && rg -q '"network_state":"not_modified"' docs/MESH_ROUTE_EXPLAIN.json; then
...
Checks:
- Git tree hygiene guard: \`${git_tree_hygiene_ok}\`
...
- Runtime real-world datapath probe error: \`${runtime_real_world_datapath_probe_error}\`
