...
if rg -q '"status":"ok"' docs/RUNTIME_REAL_WORLD_PROBE_SMOKE.json && rg -q '"kind":"runtime_real_world_probe_smoke"' docs/RUNTIME_REAL_WORLD_PROBE_SMOKE.json && rg -q '"network_state":"not_modified"' docs/RUNTIME_REAL_WORLD_PROBE_SMOKE.json; then
  ...
  # Valid real-world probe requires ordinary app HTTP(S) checks without proxy flags.
  if [[ "$runtime_real_world_skipped_no_curl" == "false" && "$runtime_real_world_direct_probe_ok" == "true" && "$runtime_real_world_datapath_probe_attempted" == "true" && "$runtime_real_world_datapath_probe_ok" == "true" && "$runtime_real_world_datapath_targets_total" -gt 0 && "$runtime_real_world_datapath_targets_failed" -eq 0 ]]; then
