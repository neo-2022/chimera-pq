...
EOF
  ...
  grep -q "^sudo -n rm -f $state_file $state_file.flow.json\$" "$sudo_log" || fail "systemd_cli_privileged_up_path: state cleanup did not use sudo"
  grep -q "^rm -f $state_file $state_file.flow.json\$" "$rm_log" || fail "systemd_cli_privileged_up_path: rm wrapper did not see state cleanup"
...
EOF
  ...
  grep -q '^sudo -n rm -f '"$state_file"' '"$state_file"'.flow.json$' "$sudo_log" || fail "systemd_stale_tun_preflight_cleanup: state cleanup did not use sudo"
