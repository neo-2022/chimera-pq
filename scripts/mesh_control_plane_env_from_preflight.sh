...
if [[ -z "$remote_peer_spec" ]]; then
  ...
  remote_region="$(trim_ascii "${CHIMERA_MESH_REMOTE_REGION:-eu}")"
  remote_load="$(trim_ascii "${CHIMERA_MESH_REMOTE_LOAD_SCORE:-20}")"
  remote_reliability="$(trim_ascii "${CHIMERA_MESH_REMOTE_RELIABILITY_SCORE:-90}")"
