...
propagate_preserved_peer_env_kv() {
  local key="${1:?key_required}"
  local value=""
  value="${!key:-}"
  if [[ -n "$value" ]]; then
    upsert_env_kv "$PEER_EGRESS_ENV_FILE" "$key" "$value"
    return 0
  fi
  value="$(read_existing_peer_env_kv "$key")"
  if [[ -n "$value" ]]; then
    upsert_env_kv "$PEER_EGRESS_ENV_FILE" "$key" "$value"
  fi
}

write_preserved_peer_env_kvs() {
  local key
  for key in \
  ...
  do
    propagate_preserved_peer_env_kv "$key"
  done
}
