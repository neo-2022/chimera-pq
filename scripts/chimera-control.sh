...
CHIMERA_MESH_SELF_NODE_ID
CHIMERA_MESH_ANNOUNCEMENT_KEYRING
CHIMERA_MESH_ANNOUNCEMENT_SIGNING_KEY
CHIMERA_PEER_UPDATE_BASE_URL
CHIMERA_PEER_UPDATE_LISTEN
EOF
}
...
configure_peer_egress_dynamic_lanes_from_bootstrap() {
  ...
  if [[ -n "${CHIMERA_MESH_ANNOUNCEMENT_KEYRING:-}" ]]; then
    upsert_env_kv "$PEER_EGRESS_ENV_FILE" "CHIMERA_MESH_ANNOUNCEMENT_KEYRING" "$CHIMERA_MESH_ANNOUNCEMENT_KEYRING"
  fi
  if [[ -n "${CHIMERA_MESH_ANNOUNCEMENT_SIGNING_KEY:-}" ]]; then
    upsert_env_kv "$PEER_EGRESS_ENV_FILE" "CHIMERA_MESH_ANNOUNCEMENT_SIGNING_KEY" "$CHIMERA_MESH_ANNOUNCEMENT_SIGNING_KEY"
  fi
  return 0
}
...
uninstall_install_artifacts() {
  ...
  if is_chimera_bootstrap_script "$bootstrap_script" && ! self_executable_path_matches "$bootstrap_script"; then
